...
const signatureVarNames = ['signer_signature_svg', 'signer_2_signature_svg']

const signerConfigs = [
  { varName: 'signer_signature_svg', label: 'Benjamin Brookarsh' },
  { varName: 'signer_2_signature_svg', label: 'Ethan Schwartz' },
]
...
function getSignatureValue(varName: string): string {
  return variables.value.find(v => v.name === varName)?.value || ''
}

function setSignatureValue(varName: string, value: string) {
  const existing = variables.value.find(v => v.name === varName)
  if (value) {
    if (existing) {
      ...
      variables.value.push({
        name: varName,
        displayName: varName.replace(/_/g, ' '),
        value,
        category: varName.startsWith('client') ? 'client' : 'our'
      })
    }
  } else {
    const idx = variables.value.findIndex(v => v.name === varName)
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="variables.length > 0" class="space-y-3">
      ...
      <div class="space-y-4">
        <SignaturePad
          v-for="cfg in signerConfigs"
          :key="cfg.varName"
          :label="cfg.label"
          :model-value="getSignatureValue(cfg.varName)"
          @update:model-value="setSignatureValue(cfg.varName, $event)"
        />
      </div>
    </div>
    ...
    <div v-for="group in groups" :key="group.label" class="space-y-3">
      ...
      <div class="space-y-2">
        <div
          ...
          class="group"
        >
          <template v-if="!signatureVarNames.includes(variable.name)">
